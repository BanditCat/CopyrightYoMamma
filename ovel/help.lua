package.preload[ "ovel.help" ] = function( name )
...
function state()
  return { [ "window" ] = hw.area }
end

function loadState( st )
  if type( st ) == "table" then
    if type( st.window ) == "table" then
      hw.area = st.window
    end
  end
end
...
local block = { ["cite"] = true, ["div"] = true, ["dl"] = true, ["p"] = true, ["h1"] = true, ["h2"] = true, ["h3"] = true, ["h4"] = true, ["h5"] = true, ["h6"] = true }
...
function renderXhtml( win, wid, html )
  ...
  local function render( ending, elem )
    ...
    elseif cc.hr or cc.br then
      ...
      if cc.pre then
        ...
        if ending and elem == "pre" and str:sub( -1, -1 ) ~= "\n" then
          str = str .. "\n"
        end
      else
        str = str:gsub( "%s+", " " )
        ...
        if cc.li and not cc.middle then
          cc.middle = true
          str = "\n  " .. cc.prefix .. ". " .. str
        end
      end
  ...
  while true do
    ...
    if not cur then
      ...
      render( true, ( type( stack[ depth - 1 ][ i[ depth - 1 ] ] ) == "table" and stack[ depth - 1 ][ i[ depth - 1 ] ].element ) or nil )	
      depth = depth - 1
      i[ depth ] = i[ depth ] + 1
    ...
    elseif type( cur ) == "table" then
      ...
      elseif cur.type == "element" then
        ...
        if cur.element == "em" then 
          cc.font.italic = true
        elseif cur.element == "ol" then
          cc.indent = ( cc.indent or 0 ) + 4
        elseif cur.element == "li" then
          cc.prefix = ( cc.prefix or 0 ) + 1
        elseif cur.element == "strong" then 
          cc.font.bold = true
        elseif cur.element == "pre" or cur.element == "code" then 
...
function show( top )
  helptxt.text = ""
  local cd = ovel.processGlobals[ "manual_" .. top ]
  if not cd then
    cd = ovel.loadFile( "docs\\" .. top .. ".html" )
  end
  if not cd then
    cd = ovel.loadFile( "..\\docs\\" .. top .. ".html" )
  end
  if not cd then
    helptxt:append( "Failed to load topic " .. top .. ".", ovel.warningFontStyle )
  else
    hw:hide()
    local s, h = ovel.expatParseXML( cd )
    if s then 
      renderXhtml( hw, helptxt, h )
    else
      helptxt:append( "Failed to load topic " .. top .. ".", ovel.warningFontStyle )
    end
  end
